...
export default function App() {
  ...
  const [submissions, setSubmissions] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('es_submissions') || '[]');
    } catch {
      return [];
    }
  });
  ...
  const saveSubmission = (res: any, ans: any[]) => {
    const record = {
      id: Date.now(),
    ...
    try {
      localStorage.setItem('es_submissions', JSON.stringify(updated));
    } catch {}
  };
  ...
  const startRecording = () => {
    ...
    if (!SR) {
      alert('Please use Chrome browser for voice recording.');
      return;
    }
    const rec = new SR();
    rec.lang = 'en-US';
  ...
  const stopAndSubmit = async () => {
    if (recRef.current) (recRef.current as any).stop();
    setIsRecording(false);
  ...
  const resetAll = () => {
    setPhase('landing');
    setForm({ name: '', whatsapp: '', ageGroup: null });
    setQIndex(0);
    setAnswers([]);
    setTranscript('');
    setResult(null);
  ...
  const LVLCOLOR: Record<string, string> = {
    Beginner: '#f9a825',
    Elementary: '#34d399',
    'Pre-Intermediate': '#60a5fa',
    Intermediate: '#f472b6',
    'Upper-Intermediate': '#a78bfa',
    Advanced: '#fc6d6d',
  };
  ...
  if (phase === 'adminLogin')
    return (
      <div style={P.page}>
        <div style={P.card}>
          <div style={P.centerLogo}>
            <div style={P.logoMark}>ES</div>
          </div>
          ...
          <input
            ...
            value={adminInput}
            onChange={(e: any) => setAdminInput(e.target.value)}
          />
          {adminError && (
            <p style={{ color: '#f87171', fontSize: 13 }}>{adminError}</p>
          )}
          <button
            style={P.greenBtn}
            onClick={() => {
          ...
          <button style={P.ghostBtn} onClick={() => setPhase('landing')}>
            ← Back
          </button>
        </div>
        <style>{CSS}</style>
  ...
  if (phase === 'admin')
    return (
      <div
        style={{ ...P.page, alignItems: 'flex-start', padding: '20px 16px' }}
      >
          ...
          <div style={P.adminHeader}>
            <div>
              <div style={P.logoRow2}>
                <div style={P.logoMark}>ES</div>
                <span style={P.brandAdmin}>English Solutions</span>
              </div>
              <p style={P.adminSub}>
                Placement Results — {submissions.length} submission
      ...
      <div style={P.page}>
        <div style={P.card}>
          ...
          <div style={{ ...P.classCardResult, borderColor: c + '55' }}>
            ...
            <div style={{ ...P.classNameResult, color: c }}>
              {result.recommended_class}
            </div>
          </div>
          <p style={P.thanksText}>
            Our team will contact you on WhatsApp soon! 😊
          </p>
          <button style={P.greenBtn} onClick={resetAll}>
            ← Start Over
          </button>
        </div>
        <style>{CSS}</style>
        ...
        <div style={P.card}>
          ...
          {(isRecording || transcript) && (
            <div style={P.transcriptBox}>
              <p style={{ ...P.transcriptText, opacity: transcript ? 1 : 0.5 }}>
          ...
          {isThinking ? (
            <div style={P.thinkingBox}>
              <div className="td1" style={P.tdot} />
              <div className="td2" style={P.tdot} />
              <div className="td3" style={P.tdot} />
              <span
                style={{
            ...
            <button
              style={{
                ...P.micBtn,
                background: `linear-gradient(135deg, ${ag.color}, ${ag.color}99)`,
              }}
          ...
          <p style={P.hintText}>
            {isRecording
              ? 'Speak clearly… tap ⏹ when you finish'
  ...
  return (
    <div style={P.page}>
      <div style={P.card}>
        ...
        <div style={P.ageGrid}>
          {AGE_GROUPS.map((g, i) => {
            const sel = form.ageGroup?.label === g.label;
            return (
              <button
                key={i}
                style={{
                  ...P.ageBtn,
                  borderColor: sel ? g.color : 'rgba(255,255,255,0.12)',
                  background: sel ? g.color + '22' : 'rgba(255,255,255,0.04)',
                  color: sel ? g.color : 'rgba(255,255,255,0.6)',
                }}
        ...
        <button
          style={{ ...P.greenBtn, opacity: canBegin ? 1 : 0.4 }}
          disabled={!canBegin}
          onClick={() => setPhase('test')}
        >
          Begin Placement →
        </button>
        <button style={P.adminLink} onClick={() => setPhase('adminLogin')}>
          🔐 Admin
        </button>
      </div>
      <style>{CSS}</style>
...
const P: Record<string, any> = {
  page: {
    minHeight: '100vh',
    background:
      'linear-gradient(160deg, #0a1628 0%, #0f2040 60%, #071020 100%)',
    display: 'flex',
    alignItems: 'center',
